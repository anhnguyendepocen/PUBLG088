...
You will have two days to work on the assignments, submitting knitted HTML files with your solutions via Moodle by 7pm on Thursday. We will subsequently open up our solutions to the problem sets. 
...
This exercise involves the `Auto` data set available as `Auto.csv` from the website for the main course textbook James et al. http://www-bcf.usc.edu/~gareth/ISL/data.html. Make sure that the missing values have been removed from the data. You should load that dataset as the first step of the exercise. *Hint: We used the command for that in the Introduction to R session in class today. Go back and look up how to read in a csv file.*
