---
title: "PUBLG088 Week 3: Solutions for Assignment 3"
...
**To calculate the residual error relative to the response we use the mean of the response and the RSE. The mean of mpg is ``r mean(Auto$mpg, na.rm=T)``. The RSE of the lm.fit was ``r 4.906`` which indicates a percentage error of ``r 4.906/mean(Auto$mpg, na.rm=T) * 100.0``%. The $R^2$ of the lm.fit was about ``r summary(lm.fit)$r.sq``, meaning ``r summary(lm.fit)$r.sq * 100.0``% of the variance in mpg is explained by horsepower.**
